...
while True:
    ...
    if (Hour == 23 and Minute == 59) or True:
        ...
        # Realizamos la consulta de la tabla 4
        query4 = f'SELECT * FROM {table4}'
        ...
        # Se ejecuta la consulta y se almacenan los datos en una variable
        df4 = pd.read_sql(query4, engine_db)
        ...
        with pd.ExcelWriter('Data{}.xlsx'.format(name), engine='xlsxwriter') as writer:
            ...
            df1.to_excel(writer, sheet_name='ST10A', index=False)
            # Escribimos la consulta 1 en el excel
            df2.to_excel(writer, sheet_name='ST10B', index=False)
            # Escribimos la consulta 1 en el excel
            df3.to_excel(writer, sheet_name='ST20', index=False)
            # Escribimos la consulta 1 en el excel
            df4.to_excel(writer, sheet_name='ST30', index=False)
